...
contract Ownable {
    ...
    constructor()  internal
    {
        _owner = msg.sender;
        address noAddr;
        emit ownershipTransferred(noAddr, _owner);
    }
...
contract Pausable is Ownable 
{
    ...
    {
        require(!_paused, "Contract is paused. Cannot execute transactions");
        _;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != getOwner(), "Passed address should not be the owner of the toeknId");

        // TODO require the msg sender to be the owner of the contract (?? should be owner of tokenId) or isApprovedForAll() to be true
        address tokenOwner = _tokenOwner[tokenId];
        require(((msg.sender == getOwner()) || (_operatorApprovals[tokenOwner][msg.sender])), "message sender should be owner or approved operator");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == _tokenOwner[tokenId], "From address should be owner");

        // TODO: require token is being transfered to valid address
        require(to != address(0), "Valid address not provided");
